...
public abstract class Task implements WizardListener {
	...
	private FinalizeActionListener finalListener;
	...
	public void moveForward() throws IOException, OutOfBordersException,
			InvalidTargetCellException, FontFormatException {
		...
		if (getMap()[newLocation.x][newLocation.y] instanceof CollectibleCell) {
			...
			moveListener.onEncounterCollectableCell();
		}
		...
		// if (moveListener != null)
		moveListener.onEncounterEmptyCell();
	...
	public void castDamagingSpell(DamagingSpell spell, Direction d)
			...
			NotEnoughIPException, FontFormatException {
		...
		if (map[target.x][target.y] instanceof ObstacleCell) {
			...
			getSpellListener().onCastingDamagingSpell(target.x, target.y);
			...
			if (newHp <= 0) {
				...
				getFinalListener().onChampionDeath(opponent);
			} else {
	...
	public void castRelocatingSpell(RelocatingSpell spell, Direction d,
			...
			InvalidTargetCellException, InCooldownException,
			NotEnoughIPException, OutOfRangeException, FontFormatException {
		...
		map[target.x][target.y] = new EmptyCell();
		spellListener.onRelocatingObstacle(target.x, target.y, newX, newY);
	...
	public void castHealingSpell(HealingSpell spell) throws IOException,
			InCooldownException, NotEnoughIPException, FontFormatException {
	...
	public void finalizeAction() throws IOException, FontFormatException {
	...
	public void endTurn() throws IOException, FontFormatException {
		...
		traitActivated = false;
		getFinalListener().onEndingChampionTurn();
		coolDown();
	...
	public void onSlytherinTrait(Direction d) throws IOException,
			...
			InCooldownException, FontFormatException {
	...
	public FinalizeActionListener getFinalListener() {
		return finalListener;
	}

	public void setFinalListener(FinalizeActionListener finalListener) {
	...
	public void setSpellListener(SpellListener spellListener) {
