...
public class FirstTask extends Task {
	...
	public void fire() {
		...
		Cell[][] map = getMap();
		getFinalListener().onDragonFire(markedCells.get(0).x, markedCells.get(0).y, markedCells.get(1).x, markedCells.get(1).y);
		...
		for (int i = 0; i < markedCells.size(); i++) {
			...
			if (cell instanceof ChampionCell) {
				...
				if (newHp <= 0) {
					...
					current.setHp(0);
					if(current==getCurrentChamp())
						getFinalListener().onDecreasingCurrentHP();
					map[markedCells.get(i).x][markedCells.get(i).y] = new EmptyCell();
					getChampions().remove(((ChampionCell) cell).getChamp());
					getFinalListener().onChampionDeath(current);
	...
	public void finalizeAction() throws IOException, FontFormatException {
		...
		if (current.getLocation().x == 4 && current.getLocation().y == 4) {
			...
			getFinalListener().onChampionWin((Wizard)getCurrentChamp());
