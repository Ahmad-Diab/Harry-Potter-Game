...
public class SecondTask extends Task {
	...
	public void moveForward() throws IOException, OutOfBordersException,
		...
		if (next instanceof TreasureCell
			...
			getFinalListener().onChampionWin(current);
	...
	public void encounterMerPerson() {
		...
		if (currentX + 1 <= 9
			...
			getFinalListener().onMerpersonAttack((int)((Wizard)getCurrentChamp()).getLocation().getX(),(int)((Wizard)getCurrentChamp()).getLocation().getY());
		...
		if (newHp <= 0) {
			...
			getFinalListener().onChampionDeath((Wizard)getCurrentChamp());
